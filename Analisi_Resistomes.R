...
#### Grafic barres GC i Q30
ggplot(data=LOGFILE_DNA_extractions, aes (x=ID, y=GC, fill=Loc, col=QC)) +
  geom_bar(stat="identity")+
  coord_flip()+
  geom_text(aes(label=Library_QC), hjust=-0.3, size=2.8, col="black")+
  scale_fill_discrete(name="Barri", labels=c("Besos", "Carmel", "Poblenou", "Sant Gervasi"))+
  scale_y_continuous(expand=expansion(mult=c(0, .1)))+
  scale_color_grey(name="Control de Qualitat \n(Contorn)", labels=c("No","Sí"))+
  theme_classic()


ggplot(data=LOGFILE_DNA_extractions, aes (x=ID, y=Q30, fill=Loc, col=QC)) +
  geom_bar(stat="identity")+
  coord_flip()+
  geom_text(aes(label=Library_QC), hjust=-0.3, size=2.8, col="black")+
  scale_fill_discrete(name="Barri", labels=c("Besos", "Carmel", "Poblenou", "Sant Gervasi"))+
